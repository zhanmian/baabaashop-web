...
<script>
  ...
  export default {
        ...
        methods: {
            ...
            // 获取列表数据
            getData() {
                fetchList( {page: 1, pageSize: this.page_size}).then(response=>{
            ...
            handleEdit(index, row) {
                this.$router.push({path: '/update_product', query: {id: row.id}
                })
            },
            ...
            // 确定删除
            deleteRow(){
                this.$axios.post('/delete_product', {id: this.productId}).then((response) => {
